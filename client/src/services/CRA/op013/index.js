...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/operation/013/${formattedDate}`);
...
const montantTotalCRA013 = async () => {
  const cra = await fetchDataFromApi();
  const totalMontant = cra.reduce((total, item) => total + item.montant, 0);
  return totalMontant;
};

const nombreOpCRA013 = async () => {
  const cra = await fetchDataFromApi();
  const nombreOperation = cra.reduce((total, item) => total + 1, 0);
  return nombreOperation;
};

export { nombreOpCRA013, montantTotalCRA013, montantCRA, montantAccepteListLastSixDays, nombreOpCRA};
