...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/operation/160/${formattedDate}`);
  ...
  try {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      try {
        ...
        const montantSum = cra.reduce((total, item) => total + 1, 0);
...
export { nombreRejCRA160, rejetAccepteListLastSixDays, nombreRejCRA};
