...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/operation/020/${formattedDate}`);
...
const montantTotalCRA020 = async () => {
  const cra = await fetchDataFromApi();
  const totalMontant = cra.reduce((total, item) => total + item.montant, 0);
  return totalMontant;
};

const nombreOpCRA020 = async () => {
  const cra = await fetchDataFromApi();
  const nombreOperation = cra.reduce((total, item) => total + 1, 0);
  return nombreOperation;
};

export { nombreOpCRA020, montantTotalCRA020, montantCRA, montantAccepteListLastSixDays, nombreOpCRA};
