...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/operation/060/${formattedDate}`);
...
const montantTotalCRA060 = async () => {
  const cra = await fetchDataFromApi();
  const totalMontant = cra.reduce((total, item) => total + item.montant, 0);
  return totalMontant;
};

const nombreOpCRA060 = async () => {
  const cra = await fetchDataFromApi();
  const nombreOperation = cra.reduce((total, item) => total + 1, 0);
  return nombreOperation;
};

export { nombreOpCRA060, montantTotalCRA060, montantCRA, montantAccepteListLastSixDays, nombreOpCRA};
