...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/010/${formattedDate}`);
...
const nombreOp010 = async () => {
  const lot = await fetchDataFromApi();
  const nombreOperation = lot.reduce((total, item) => total + item.nombreOperation, 0);
  return nombreOperation;
}

const montantTotal010 = async () => {
  const lot = await fetchDataFromApi();
  const totalMontant = lot.reduce((total, item) => total + item.montant, 0);
  return totalMontant;
}
 
const opLastSixDays010 = async() => {
  return await nombreOperationListLastSevenDays();
}

export {opLastSixDays010, montantTotal010, nombreOp010, nombreLot, nombreLot010, montantTotal, nombreOp, nombreOperationListLastSevenDays};
