...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/120/${formattedDate}`);
...
const nombreLot120 = async() => {
  const lot = await fetchDataFromApi();
  return lot.length;
}

const nombreRej120 = async () => {
  const lot = await fetchDataFromApi();
  const nombreRejet = lot.reduce((total, item) => total + item.nombreRejet, 0);
  return nombreRejet;
}

const opLastSixDays120 = async() => {
  return await nombreRejetListLastSevenDays();
}

export {opLastSixDays120, nombreRej120, nombreLot, nombreLot120, nombreRej, nombreRejetListLastSevenDays};
