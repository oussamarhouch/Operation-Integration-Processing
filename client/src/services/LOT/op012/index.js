...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/012/${formattedDate}`);
...
const nombreLot012 = async() => {
  const lot = await fetchDataFromApi();
  return lot.length;
}

const nombreOp012 = async () => {
  const lot = await fetchDataFromApi();
  const nombreOperation = lot.reduce((total, item) => total + item.nombreOperation, 0);
  return nombreOperation;
}

const montantTotal012 = async () => {
  const lot = await fetchDataFromApi();
  const totalMontant = lot.reduce((total, item) => total + item.montant, 0);
  return totalMontant;
}

const opLastSixDays012 = async() => {
  return await nombreOperationListLastSevenDays();
}

export {opLastSixDays012, montantTotal012, nombreOp012, nombreLot,nombreLot012, montantTotal, nombreOp, nombreOperationListLastSevenDays};
