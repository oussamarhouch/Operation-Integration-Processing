...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/060/${formattedDate}`);
...
const nombreLot060 = async() => {
  const lot = await fetchDataFromApi();
  return lot.length;
}

const nombreOp060 = async () => {
  const lot = await fetchDataFromApi();
  const nombreOperation = lot.reduce((total, item) => total + item.nombreOperation, 0);
  return nombreOperation;
}

const montantTotal060 = async () => {
  const lot = await fetchDataFromApi();
  const totalMontant = lot.reduce((total, item) => total + item.montant, 0);
  return totalMontant;
}

const opLastSixDays060 = async() => {
  return await nombreOperationListLastSevenDays();
}

export {opLastSixDays060, montantTotal060, nombreOp060, nombreLot, nombreLot060, montantTotal, nombreOp, nombreOperationListLastSevenDays};
