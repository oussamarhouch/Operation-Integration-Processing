...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/031/${formattedDate}`);
...
const nombreLot031 = async() => {
  const lot = await fetchDataFromApi();
  return lot.length;
}

const nombreOp031 = async () => {
  const lot = await fetchDataFromApi();
  const nombreOperation = lot.reduce((total, item) => total + item.nombreOperation, 0);
  return nombreOperation;
}

const montantTotal031 = async () => {
  const lot = await fetchDataFromApi();
  const totalMontant = lot.reduce((total, item) => total + item.montant, 0);
  return totalMontant;
}

const opLastSixDays031 = async() => {
  return await nombreOperationListLastSevenDays();
}

export {opLastSixDays031, montantTotal031, nombreOp031, nombreLot, nombreLot031, montantTotal, nombreOp, nombreOperationListLastSevenDays};
