...
const fetchDataFromApi = async () => {
  try {
    const response = await axios.get(`${API_URL}/020/${formattedDate}`);
...
const nombreLot020 = async() => {
  const lot = await fetchDataFromApi();
  return lot.length;
}

const nombreOp020 = async () => {
  const lot = await fetchDataFromApi();
  const nombreOperation = lot.reduce((total, item) => total + item.nombreOperation, 0);
  return nombreOperation;
}

const montantTotal020 = async () => {
  const lot = await fetchDataFromApi();
  const totalMontant = lot.reduce((total, item) => total + item.montant, 0);
  return totalMontant;
}

const opLastSixDays020 = async() => {
  return await nombreOperationListLastSevenDays();
}

export {opLastSixDays020, montantTotal020, nombreOp020, nombreLot, nombreLot020, montantTotal, nombreOp, nombreOperationListLastSevenDays};
